...
void Widget_PLCPicture::setLabelText()
{
	...
	for (int i=0;i<12;i++)
	{
		...
		QString tempText = QString::fromLocal8Bit(PLCStatusiniset.value(strSession,"reserve").toString());
	...
	for (int i=0;i<6;i++)
	{
		...
		listPLCInfoIN1.append(QString::fromLocal8Bit(PLCStatusiniset.value(strSession,"reserve").toString()));//.toLatin1().data()));
		listSensorTextIN1.at(i)->setText(listPLCInfoIN1.at(i));
	}
	for (int i=0;i<8;i++)
	{
		strSession = QString("/IO/Y00%1").arg(i);
		listPLCInfoOUT0.append(QString::fromLocal8Bit(PLCStatusiniset.value(strSession,"reserve").toString()));//.toLatin1().data()));
		listSensorTextOUT0.at(i)->setText(listPLCInfoOUT0.at(i));
	}
	for (int i=0;i<4;i++)
	{
		strSession = QString("/IO/Y10%1").arg(i);
		listPLCInfoOUT1.append(QString::fromLocal8Bit(PLCStatusiniset.value(strSession,"reserve").toString()));//.toLatin1().data()));
